...
team_box_games <- purrr::map_dfr(sort(years_vec, decreasing = TRUE), function(y){
  ...
  write.csv(team_box_g, file=gzfile(glue::glue("mbb/team_box/csv/team_box_{y}.csv.gz")), row.names = FALSE)
...
write.csv(sched_g %>% dplyr::arrange(desc(.data$date)), 'mbb_schedule_master.csv', row.names = FALSE)
